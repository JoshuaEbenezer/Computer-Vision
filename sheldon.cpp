...
void classify(char* oper){
	...
	for (int i = 0; i<strlen(oper); i++){
		...
		case '/':ops[j]='/'; j++; break;
		case '*':ops[j] = '*'; j++; break;
		default:numbers[k] = (oper[i]); k++; break;
		}
	}
}
...
int linear2[50] = { ' ' };
	...
	do{
		...
		} while (next_permutation(linear1, linear1 + 50);
	} while (next_permutation(linear2, linear2 + 50);
